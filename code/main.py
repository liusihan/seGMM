...
def collect_SRY(bamfile,quality,genome_version,outdir):
    ...
    runcmd(cmd)
    cmd="cat "+bamfile+''' | awk '{print $1}' | while read id; do cat '''+outdir+"/SRY/$id.mosdepth.summary.txt | "+'''awk 'BEGIN{OFS="\\t"}NR==5{print "'$id'",$4}'; done | sort -n -k 1 >''' + outdir+"/SRY.txt"
    runcmd(cmd)
